...
def fluka_plane_string(SurfaceCard):
    ...
    mag = a**2 + b**2 + c**2
    x_p = a * (a * 0.0 + b * 0.0 + c * 0.0 + d) / mag
    y_p = b * (a * 0.0 + b * 0.0 + c * 0.0 + d) / mag
    z_p = c * (a * 0.0 + b * 0.0 + c * 0.0 + d) / mag
...
def fluka_plane_x_string(SurfaceCard):
    string = (
        "YZP S"
        + str(SurfaceCard.surface_id)
        + " "
        + str(SurfaceCard.surface_coefficients[3])
        + "\n"
    )
    return string
...
def fluka_cylinder_x(SurfaceCard):
    string = (
        ...
        + str(SurfaceCard.surface_id)
        + " "
        + str(SurfaceCard.surface_coefficients[0])
        + " "
    )
...
def fluka_cylinder_y(SurfaceCard):
    string = (
        ...
        + str(SurfaceCard.surface_id)
        + " "
        + str(SurfaceCard.surface_coefficients[1])
        + " "
    )
...
def fluka_cone_x(SurfaceCard):
    ...
    if SurfaceCard.surface_coefficients[4] != 0.0:
        ...
        # cone points up from xyz
            ...
            h = -1.0 * (SurfaceCard.b_box[1] - SurfaceCard.surface_coefficients[0])
        ...
        coefficients[4] = 0.0
        coefficients[5] = 0.0
        coefficients[6] = -2.0 * t * x_bar
        coefficients[7] = -2.0 * y_bar
        coefficients[8] = -2.0 * z_bar
...
def fluka_cone_y(SurfaceCard):
    ...
    if SurfaceCard.surface_coefficients[4] != 0.0:
        ...
        # cone point up from xyz
            ...
            h = -1.0 * (SurfaceCard.b_box[3] - SurfaceCard.surface_coefficients[1])
    ...
    # function
        ...
        coefficients[4] = 0.0
        coefficients[5] = 0.0
        coefficients[6] = -2.0 * x_bar
        coefficients[7] = -2.0 * t * y_bar
        coefficients[8] = -2.0 * z_bar
...
def fluka_cone_z(SurfaceCard):
    ...
    if SurfaceCard.surface_coefficients[4] != 0.0:
        ...
        # cone point up from xyz
            ...
            h = -1.0 * (SurfaceCard.b_box[5] - SurfaceCard.surface_coefficients[2])
...
def fluka_torus_x(SurfaceCard):
    warnings.warn(
        "Warning tori are not supported in FLUKA, some manual manipulation will be required",
        Warning,
    )
