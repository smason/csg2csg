...
from csg2csg.SerpentSurfaceCard import write_serpent_surface
from csg2csg.SerpentCellCard import write_serpent_cell
...
class SerpentInput(InputDeck):
    ...
    def write_serpent(self, filename, flat=True):
        with open(filename, "w") as fd:
            self._write_serpent_surfaces(fd)
            self._write_serpent_cells(fd)
            self._write_serpent_materials(fd)
