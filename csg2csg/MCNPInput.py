...
class MCNPInput(InputDeck):
    ...
    def __process_importances(self):
        # process the lists
        for particle, importance in self.importance_list.items():
            # early exit
            if "i" not in importance and "r" not in importance:
                # TODO: should this really be return rather than continue?
                return
    ...
    def __get_importances(self, start_line):
        ...
        while True:
            ...
            particle = line.split()[0].split(":")[1]

            # TODO mcnp allows the following forms imp:n imp:n,p etc
            particle = mcnpToParticle(particle)
            logging.debug(
                "found importance statement for particle %s on line %s",
                particleToGeneric(particle), idx
            )

            self.importance_list[particle] = line[5:].rstrip()
            idx += 1
            # while we find a continue line
            while self.file_lines[idx][0:5] == "     ":
                self.importance_list[particle] += self.file_lines[idx].rstrip()
                logging.debug(
                    "importance statement has continue line %s", str(idx)
                )
                idx += 1
    ...
    def __get_material_cards(self, start_line):
        ...
        while True:
            ...
            if re.match(" *m[0-9]/*", self.file_lines[idx]):
                #            if "m" in self.file_lines[idx] and not any(x in self.file_lines[idx] for x in MCNP_MATERIAL_KEYWORDS):
    ...
    def __reorganise_materials(self):
        ...
        for cell in self.cell_list:
            ...
            if cell.cell_material_number in material_density:
                ...
                if not any(
                    ...
                    for density in material_density[cell.cell_material_number]
                ):
                    material_density[cell.cell_material_number].append(
                        cell.cell_density
                    )
            else:
        ...
        if 0 in material_density.keys():
            del material_density[0]

        # maybe void problem
        if len(material_density) == 0:
            return
            ...
            if num_densities > 1:
                ...
                for density in material_density[mat][1:]:
                    ...
                    material.material_name = (
                        "Copy of Material " + str(mat) + " with density "
                    )
                    ...
                    for idx in range(len(self.cell_list)):
                        ...
                        # if we match
                        if (
                            cell.cell_material_number == mat
                            and cell.cell_density == density
                        ):
    ...
    def __make_new_plane(self, vector, offset):
        ...
        surf = MCNPSurfaceCard(
            surface_string
        )  # todo maybe instanciate explicitly generically?
        return surf
    ...
    def __macro_rcc_cylinder_arbitrary(self, Surface, vector):
        ...
        # plane offset 1
        d1 = (
            axis_vector[0] * Surface.surface_coefficients[0]
    ...
    def __simplify_cone(self, surf):
        coeff = surf.surface_coefficients
        ...
        new_surf = MCNPSurfaceCard(name)
        oldid, newid = surf.surface_id, new_surf.surface_id

        # make a new ambiguity surface
        # TODO: the spacing in the following seems kind if arbitrary,
        # am keeping because it's tested for, but does it matter?
        if coeff[4] == -1:
            cell_description_inside = f"( -{oldid} -{newid} )"
            cell_description_outside = f"(  {oldid} -{newid} : {newid})"
        elif coeff[4] == 1:
            cell_description_inside = f"( -{oldid} {newid} )"
            cell_description_outside = f"( {oldid} {newid} : -{newid} )"
        else:
            raise ValueError(f"Unknown surface coefficient {coeff[4]=!r}")

        # update the cone description
        print(coeff)
        surf.surface_coefficients = coeff[0:4]

        cell_description = [cell_description_inside, cell_description_outside]
        return cell_description, [new_surf]
    ...
    def __simplify_cones(self):
        ...
        for surf in self.surface_list:
            ...
            print(
                surf.surface_type,
                surf.surface_type in SURFACE_CONE_TYPES
            )
            if surf.surface_type not in SURFACE_CONE_TYPES:
                continue
            ...
            regex = re.compile(f"^-?({surf.surface_id})(\\.+[1-9])?$")
            outside = f"{surf.surface_id}"
            inside = f"-{surf.surface_id}"
            ...
            for jdx, cell in enumerate(self.cell_list):
                ...
                description = cell.cell_text_description

                # we're done when the macrobdy surface id doesn't appear in the text
                while any(map(regex.search, description)):
                    # loop over each component and find the macrobody
                    for idx, surface in enumerate(description):
                        # if it matches we have the simmple form
                        if surface == outside:
                            description[idx] = cell_description[1]
                        elif surface == inside:
                            description[idx] = cell_description[0]

                cell.cell_text_description = description

                # update the text description
                text_string = " ".join(description)
    ...
    def __flatten_macrobodies(self):
        ...
        for surf in self.surface_list:
            ...
            if surf.is_macrobody():
                ...
                logging.debug(
                    "%s %i %s", "Surface", surf.surface_id, "is to be deleted"
                )
                        ...
                        if matches:
                            ...
                            for idx, surface in enumerate(cell.cell_text_description):
                                ...
                                if str(surf.surface_id) == surface:
                                    # replace it
                                    cell.cell_text_description[idx] = cell_description[
                                        1
                                    ]
                                ...
                                if str(surf.surface_id) + "." in surface:
                                    surface_index = int(
                                        surface.split(".")[1]
                                    )  # get just the mcnp surface index
                                    new_surface_id = new_surfaces[
                                        surface_index - 1
                                    ].surface_id  # mcnp numbers them 1->n
                                    if "-" in surface:  # need to take care of the -sign
                                        cell.cell_text_description[idx] = "-" + str(
                                            new_surface_id
                                        )
                                    else:
                                        cell.cell_text_description[idx] = str(
    ...
    def __remove_inline_comment(self, cell_line):
        if "$" in cell_line:
            ...
            if line_out.isspace() and cell_comment:
                print(
                    line_out, cell_comment, line_out.isspace(), cell_comment.isspace()
                )
            return line_out
        else:
            return cell_line
    ...
    def __get_cell_cards(self):
        ...
        while True:
            ...
            while True:
                # remove inline comment ($) if needed
                self.file_lines[jdx] = self.__remove_inline_comment(
                    self.file_lines[jdx]
                )
                cell_line = self.file_lines[jdx]
    ...
    def __remove_duplicate_surfaces(self):
            ...
            for surf in surf_by_type[type]:
                ...
                for compare in surf_by_type[type]:
                    ...
                    if same:
                        ...
                        logging.debug(
                            "%s",
                            "Surface %s and Surface %s match with the sense %s"
                            % (match.surface_id, original.surface_id, reverse),
                        )
        ...
        for cell in self.cell_list:
            ...
            for surf in duplicates.keys():
                ...
                # rather than the id
                cell.replace_surface(
                    duplicates[surf].surface_id, surf.surface_id, senses[surf]
                )
