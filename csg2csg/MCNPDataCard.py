...
class MCNPTransformCard(MCNPDataCard):
    ...
    def __process_string(self):
        ...
        self.shift = [float(tokens[1]), float(tokens[2]), float(tokens[3])]

        if len(tokens) == 13 or len(tokens) == 14:  # fully defined transform
            self.v1 = [float(tokens[4]), float(tokens[5]), float(tokens[6])]
            self.v2 = [float(tokens[7]), float(tokens[8]), float(tokens[9])]
            self.v3 = [float(tokens[10]), float(tokens[11]), float(tokens[12])]

            # convert from degs to radians
            if self.angle_form:
                for i in range(3):
                    self.v1[i] = math.cos(self.v1[i] / 180.0 * math.pi)
                    self.v2[i] = math.cos(self.v2[i] / 180.0 * math.pi)
                    self.v3[i] = math.cos(self.v3[i] / 180.0 * math.pi)
